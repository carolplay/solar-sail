...
Target Version: Demo 0.1.0, with Demo 0.2.0 upgrade path

Demo 0.1.0 is a browser visualization prototype, not a spacecraft navigation tool. The implementation favors credible intuition, stable performance, and clear documentation over high-precision astrodynamics.
...
Demo 0.1.0 fit: good later, not required now
...
Demo 0.1.0 fit: good candidate for Demo 0.2.0+
...
Demo 0.1.0 fit: useful for future route research

### Browser-Side Keplerian Approximation

This is the current Demo 0.1.0 approach. Each displayed body uses simplified two-body Keplerian elements: semi-major axis, eccentricity, orbital period, phase, and argument of periapsis. The app solves Kepler's equation with a small Newton iteration and renders approximate true anomaly positions.

Complexity: low  
Browser feasibility: high  
Accuracy: approximate  
Demo 0.1.0 fit: best current fit
...
Demo 0.1.0 fit: strong upgrade path

## Current Demo 0.1.0 Model
...
The Demo 0.1.0 conceptual views now follow the visual language in Luke Chu's low-energy transfer explainer more closely: pseudo-3D effective-potential terrain, saddle regions around Lagrange points, halo/Lyapunov-style loops, and stable/unstable tube families. The implementation still does not numerically solve the circular restricted three-body problem. It draws symbolic geometry intended to teach the idea of energy terrain and transfer corridors.
...
The first implementation pass used a stylized 780-day cycle with a 258-day Earth-to-Mars rendezvous offset. That was too loose. The revised Demo 0.1.0 model uses the commonly cited Aldrin-cycler simplification: one Earth-Mars synodic period per cycle, about 2.135 Earth years or roughly 780 days, and about 146 days for the Earth-to-Mars crossing.

The visible shuttle/taxi transfers remain conceptual. In a real cycler architecture, short taxis must perform demanding rendezvous and departure burns near planetary flybys; Demo 0.1.0 shows them as infrastructure timing markers rather than physically integrated transfer vehicles.

## Recommendation

Keep Demo 0.1.0 on browser-side Keplerian approximations and symbolic gateway geometry. The result is lightweight, explainable, and visually responsive. For a later demo, add an offline ephemeris generation step using JPL Horizons or Skyfield and load the generated JSON in the app.
